...
for (int x = 0; x < copia.length - 1; x++) {
	...
	public boolean alterarEndereco(int i, String endereco) {
		if (endereco != null) {
			for (int j = 0; j < alunos.length; j++) {
				if (alunos[j] != null && alunos[j].getCodigo() == i) {
					alunos[j].setEndereco(endereco);
					return true;
				}
			}
		}
	...
	public boolean alterarNomeMae(int i, String NomeMae) {
		if (NomeMae != null) {
			for (int j = 0; j < alunos.length; j++) {
				if (alunos[j] != null && alunos[j].getCodigo() == i) {
					alunos[j].setNmMae(NomeMae);
					return true;
				}
			}
		}
